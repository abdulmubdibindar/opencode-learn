...
**Claude Code:** copy this folder to `~/.claude/skills/icm-architecture/` (or `.claude/skills/icm-architecture/` inside a project), then ask Claude to "ICM this" / "structure this for agents" / "build me a workspace for X".

**Claude apps:** upload `icm-architecture.skill` (build it with the skill-creator packager, or zip this folder's contents) via Settings → Capabilities.
